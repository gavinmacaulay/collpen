function [xs ys us vs snrs pkhs is] = JC_PIV_getPIVvectors(folder, avifilename, denoising_method, denoising_param, denoising_label, parstr)
%parstr = struct('showmsg',1,'winsize',16,'olap',0.5,'write',1,'useold',0);
...
[xs ys us vs snrs pkhs is] = ...
    PIV_getSubRawPIVvectors(folder, avifilename, denoising_method, denoising_param, denoising_label, parstr);
...
function [xs ys us vs snrs pkhs is] = PIV_getSubRawPIVvectors(folder, avifilename, denoising_method, denoising_param, denoising_label, parstr)
% dx     : 1 or 0
% dy     : 1 or 0
% winsize: 32 64 128
...
if(denoising_method ~= -1) % apply denoising
    %% Load Bg image
...
for i=1:1:n
    ...
    c=c+1;
    if c>30
        dispMsg(parstr.showmsg,['[PIV_getRawPIVvectors]:....Frame: ' num2str(i) ' of ' num2str(n) ' analysed']); 
        c=0;
    end
