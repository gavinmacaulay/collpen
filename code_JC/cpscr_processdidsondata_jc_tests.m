...
addpath('/Volumes/Datos/collpen/RedSlip')
savepath

d=dir('/Volumes/Datos/collpen/RedSlip/*%.avi');
...
for i=1:length(d)
    ...
    par(i).msnrs = 1.3;
    par(i).ssnrs = .2;
    filedir{i} = '/Volumes/Datos/collpen/RedSlip';
    file{i} = d(i).name;
end
...
for i=1:length(d)
    ...
    parstrbgimage       = struct('showmsg',1,'Nframes',200,'perc',70,'write',1,'useold',0);
