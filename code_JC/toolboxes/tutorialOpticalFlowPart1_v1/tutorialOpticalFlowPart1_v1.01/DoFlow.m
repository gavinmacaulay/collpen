...
%     method = 'LK';         %% Lucas Kanade
    method = 'NOTHING';      %% Do nothing, i.e. return zeros
      ...
      vflowRes = 25; %resolution of the flow field in the image
      hflowRes = 25;
      %     MOMENT CALCULATIONS
      gaussStd = 2; % for tensor smoothing.
      gg  = single(gaussgen(gaussStd)); %% filter for tensor smoothing

 m200=imresizeNN(conv2(gg,gg, dx.^2 ,'same'),[hflowRes vflowRes]);
 m020=imresizeNN(conv2(gg,gg, dy.^2 ,'same'),[hflowRes vflowRes]);
 m110=imresizeNN(conv2(gg,gg, dx.*dy,'same'),[hflowRes vflowRes]);
 m101=imresizeNN(conv2(gg,gg, dx.*dt,'same'),[hflowRes vflowRes]);
 m011=imresizeNN(conv2(gg,gg, dy.*dt,'same'),[hflowRes vflowRes]);
