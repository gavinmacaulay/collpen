function [xs ys us vs snrs pkhs is] = getPIVvectorsFromVideo(folder, avifilename, parstr)

% This function calculates the PIV vectors for a given input file. 

dispMsg(parstr.showmsg,'[getPIVvectorsFromVideo]: Start');
...
elseif nargin==6
    ...
    dispMsg(parstr.showmsg,'[getPIVvectorsFromVideo]: End');
    return;
end
...
if ~(exist(datafolder,'dir')==7)
    dispMsg(parstr.showmsg,['[getPIVvectorsFromVideo]: Creating data folder, ' datafolder]);
    mkdir(datafolder);
end
...
if exist(datapath, 'file') == 2 & currentparstr.useold
    load(datapath);
    pos = -1;
    for i=1:length(pivdatas)
        ...
        if compareStruct(currentparstr,parstr)
            ...
            if currentparstr.useold
                dispMsg(parstr.showmsg,'[getPIVvectorsFromVideo]: ..Using previously created PIVs');
...
dt      = 1;%.0/8.0; % Herring videos were recorded at 8fps
rows    = 1;
cols    = 1;
...
for i=1:1:n-1
    ...
    % Loading frames
    I2    = read(movieobj, i+1);
    [r c nc] = size(I2);
    if nc>1
        I2      = rgb2gray(I2);
    end

    % Ouput to show that something is happening
    c=c+1;
    if c>30
        dispMsg(parstr.showmsg,['[PIV_getRawPIVvectors]:....Frame: ' ...
            num2str(i) ' of ' num2str(n) ' analysed']); 
        c=0;
    end
    
    % PIV
    try
        [T,xs(:,:,i),ys(:,:,i),us(:,:,i),vs(:,:,i),snrs(:,:,i),pkhs(:,:,i)] ...
            = evalc('matpivCP(I1,I2,winsize,dt,olap,param)');
        tmpx=xs(1,:,i);
        tmpy=ys(:,1,i);
        is(:,:,i)=If(tmpy(:),tmpx(:));
    catch exception
        warning(['[PIV_getRawPIVvectors]: Error in matpiv for frames ' ...
            num2str(i) ' and ' num2str(i+1)]);
...
function [folder, avifilename,parstr]=checkingArguments(folder, ...
    avifilename,parstr)
avifilename=strrep([avifilename '.avi'],'.avi.avi','.avi');
dparstr = struct('showmsg',1,'winsize',64,'olap',0.5,'write',1,'useold',1,...
    'medianfilter',0,'bgfilter',0);
    ...
    for ii=1:(1-overlap)*winsize:size(A,2)-winsize+1
        if IN(jj+N/2,ii+M/2)~=1
            ...
            if isnan(stad1)~=1 & isnan(stad2)~=1
                ...
                u(cj1,ci1)=NaN; v(cj1,ci1)=NaN; SnR(cj1,ci1)=NaN; ...
