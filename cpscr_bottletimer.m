function bottleTimer()
    
   % Code to provide an audio countdown for releasing the bottle
   
   w = 30; % How long to wait before the beep [s]
   logFilename = 'bottleReleases.txt'; % log file for release times
   
   txt = 'Starting countdown';
   disp(txt)
   tts_local(txt)
   
   intervals = [w:-5:5 4 3 2 1];
   p = abs([0 diff(intervals)]);
   ...
   for i = 1:length(intervals)
       ...
       if intervals(i) > 5
           txt = [num2str(intervals(i)) ' seconds'];
       else
           txt = num2str(intervals(i));
       end
       disp(txt)
       tts_local(txt)
   end
   ...
   txt = ['Beep at ' datestr(d, 'yyyymmdd-THHMMSS.FFF')];
   disp(txt)

   fid = fopen(logFilename, 'a+');
   
   if fid == -1
       warning(['Could not open file ' filename ' for writing'])
   else
       fprintf(fid, '%s\n', txt);
   end
...
% Embedded version of tts, obtained from the Mathworks File Exchange
function wav = tts_local(txt,voice,pace,fs)
%TTS text to speech.
